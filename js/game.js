class Game {
    ...
    async start() {
        ...
        player1 = createSprite(displayWidth / 2, displayHeight - 150);
        player1.addImage("player1", player_img);

        player2 = createSprite(displayWidth / 2, displayHeight - 150);
    ...
    play() {
        ...
        if (frameCount % 20 === 0) {
            fruits = createSprite(random(displayWidth / 10, displayWidth), 0, 100, 100);
            fruits.velocityY = 10;
        ...
        if (player.index !== null) {
            ...
            for (plr in allPlayers) {
                fill("white");
                textSize(30);
                text(allPlayers[plr].name + "  : " + allPlayers[plr].score, 40, textY);
                textY += 30;
            }
        }
